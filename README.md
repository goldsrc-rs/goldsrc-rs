...
│   └── goldsrc-core/               # Host runtime orchestrator, engine bridge, SQLite storage, i18n, CLI
├── backends/
│   ├── goldsrc-backend-metamod/    # Metamod-r C-ABI plugin adapter (Meta_Query, Meta_Attach)
│   └── goldsrc-backend-standalone/ # Standalone proxy GameDLL adapter (GiveFnptrsToDll, GetEntityAPI2)
├── hosts/
│   └── goldsrc-host-wasm/          # Embedded Wasmtime runtime, JIT engine, and plugin manager
├── framework/
